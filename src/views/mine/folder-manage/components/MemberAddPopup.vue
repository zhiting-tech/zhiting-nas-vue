...
export default {
  ...
  computed: {
    choseList() {
      return this.memberList.filter(item => item.checked && !item.disabled)
    },
    hasChose() {
  ...
  methods: {
    ...
    // 保存逻辑
    onClickRight() {
      this.$emit('change', this.choseList)
      this.show = false
    },
    initParams() {
      this.memberList.forEach((item) => {
        const isExist = this.params.list.find(user => user.u_id === item.u_id)
        if (isExist) {
          item.checked = true
          item.disabled = true
        } else {
    ...
    handelChecked(member) {
      const isExist = this.memberList.find(item => item.checked)
      if (this.params.private && isExist && !member.checked) {
        this.$toast(this.$t('folder.privateLimit'))
        return
      }
      if (this.params.private) {
        member.read = 1
        member.write = 1
        member.deleted = 1
      } else {
